...
export const normalizeFileUrl = (url) => {
  ...
  // /investor/ files are static — served directly from public/
  if (relative.startsWith('/investor/')) return relative;

  // /uploads/ files are served via the API file route (persistent outside build)
  if (relative.startsWith('/uploads/')) {
    return `/api/files${relative.slice('/uploads'.length)}`;
  }
