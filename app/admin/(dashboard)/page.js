...
import {
  ...
  Clock, Calendar, Eye, Mail, Bell, Layout, MapPin,
} from 'lucide-react';
  ...
  return (
    <Link href={href} className="bg-white p-5 rounded-2xl border border-gray-100 hover:shadow-lg hover:border-teal-100 transition-all duration-300 group relative overflow-hidden">
      ...
      <div className="flex items-center gap-4 relative z-10">
        ...
        <div className="text-zinc-200 group-hover:text-teal-500 transition-colors"><ArrowRight size={18} /></div>
      </div>
    </Link>
  );
}
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        // Only fetch what user has permission to see
        const fetches = [
          hasPermission('documents')    ? adminApi.get('/documents')           : Promise.resolve(null),
          hasPermission('news')         ? adminApi.get('/news?limit=5')        : Promise.resolve(null),
          hasPermission('board-members')? adminApi.get('/board-members')       : Promise.resolve(null),
          hasPermission('contacts')     ? adminApi.get('/contacts')            : Promise.resolve(null),
          hasPermission('careers')      ? adminApi.get('/careers')             : Promise.resolve(null),
          hasPermission('pages')        ? adminApi.get('/products')            : Promise.resolve(null),
          hasPermission('notifications')? adminApi.get('/notifications?limit=5'): Promise.resolve(null),
        ];
        ...
        if (newsData)     setRecentNews((newsData?.news || []).slice(0, 5));
        if (contactsData) setRecentContacts((contactsData?.contacts || []).slice(0, 5));
        if (notifData)    setRecentNotifications((notifData?.notifications || []).slice(0, 5));
  ...
  // Only show cards user has permission for
  const ALL_CARDS = [
    { permission: 'documents',     icon: FileText,      label: 'Docs',      value: stats.docs,     color: 'bg-emerald-500', sub: 'PDF Reports',               href: '/admin/documents' },
    { permission: 'news',          icon: Newspaper,     label: 'Media',     value: stats.news,     color: 'bg-teal-500',    sub: 'Latest News',               href: '/admin/news' },
    { permission: 'board-members', icon: Users,         label: 'Board',     value: stats.members,  color: 'bg-emerald-600', sub: 'Directors',                  href: '/admin/board-members' },
    { permission: 'pages',         icon: Package,       label: 'Inventory', value: stats.products, color: 'bg-green-500',   sub: 'VIL Products',              href: '/admin/products' },
    { permission: 'careers',       icon: Briefcase,     label: 'Jobs',      value: stats.jobs,     color: 'bg-cyan-600',    sub: 'Hiring',                     href: '/admin/careers' },
    { permission: 'contacts',      icon: MessageSquare, label: 'Inbox',     value: stats.contacts, color: 'bg-emerald-400', sub: stats.unread ? `${stats.unread} New` : 'Clean', href: '/admin/contacts' },
  ];
  const visibleCards = ALL_CARDS.filter(c => hasPermission(c.permission));

  // Quick actions — only for permitted modules
  const ALL_QUICK = [
    { permission: 'documents', label: 'Upload PDF',  href: '/admin/documents',  icon: FileText,      color: 'group-hover:bg-emerald-500' },
    { permission: 'news',      label: 'Add News',    href: '/admin/news',       icon: Newspaper,     color: 'group-hover:bg-teal-500' },
    { permission: 'contacts',  label: 'Check Mail',  href: '/admin/contacts',   icon: MessageSquare, color: 'group-hover:bg-blue-500' },
    { permission: 'careers',   label: 'Post Job',    href: '/admin/careers',    icon: Briefcase,     color: 'group-hover:bg-cyan-600' },
    { permission: 'board-members', label: 'Board',   href: '/admin/board-members', icon: Users,     color: 'group-hover:bg-emerald-600' },
    { permission: 'pages',     label: 'Edit Pages',  href: '/admin/pages',      icon: Layout,        color: 'group-hover:bg-violet-500' },
  ];
  const visibleQuick = ALL_QUICK.filter(q => hasPermission(q.permission)).slice(0, 4);
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <div className="flex items-center gap-4 bg-zinc-50/80 border border-zinc-100 p-4 rounded-2xl">
          ...
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-2 text-[11px] font-bold text-zinc-400 uppercase tracking-wider">
              <Calendar size={12} className="text-zinc-300" />{dateString}
            </div>
          </div>
        </div>
      </div>
      ...
      {visibleCards.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {loading
            ? [...Array(visibleCards.length)].map((_, i) => <div key={i} className="h-24 bg-white border border-zinc-100 rounded-2xl animate-pulse" />)
            : visibleCards.map(c => <StatCard key={c.label} {...c} />)}
        </div>
      )}

      {/* Recent Activity — only show sections user has access to */}
      {(hasPermission('news') || hasPermission('contacts')) && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white border border-gray-100 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-5">
                <h2 className="font-black text-gray-900 flex items-center gap-2">
                  <div className="w-2 h-5 bg-teal-500 rounded-full" /> Recent News
                </h2>
                <Link href="/admin/news" className="text-[10px] font-black text-teal-600 uppercase tracking-wider hover:opacity-70 flex items-center gap-1">
              ...
              {loading ? (
                ...
                <div className="text-center py-8"><Newspaper size={28} className="mx-auto text-gray-200 mb-2" /><p className="text-sm text-gray-400">No news articles yet</p></div>
              ) : (
                <div className="space-y-3">
                  {recentNews.map(item => (
                    <div key={item._id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-gray-50 transition-colors">
                      <div className="w-10 h-10 bg-teal-50 rounded-lg flex items-center justify-center flex-shrink-0"><Newspaper size={16} className="text-teal-500" /></div>
                      <div className="flex-1 min-w-0">
          ...
          {hasPermission('contacts') && (
            <div className="bg-white border border-gray-100 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-5">
                <h2 className="font-black text-gray-900 flex items-center gap-2">
                  <div className="w-2 h-5 bg-emerald-500 rounded-full" /> Recent Messages
                </h2>
                <Link href="/admin/contacts" className="text-[10px] font-black text-teal-600 uppercase tracking-wider hover:opacity-70 flex items-center gap-1">
                  View All <ArrowRight size={12} />
                </Link>
              </div>
              {loading ? (
                ...
                <div className="text-center py-8"><Mail size={28} className="mx-auto text-gray-200 mb-2" /><p className="text-sm text-gray-400">No messages yet</p></div>
              ) : (
                <div className="space-y-3">
      ...
      {/* Recent Notifications — only if permitted */}
      {hasPermission('notifications') && (
        <div className="bg-white border border-gray-100 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-5">
            <h2 className="font-black text-gray-900 flex items-center gap-2">
              <div className="w-2 h-5 bg-orange-400 rounded-full" /> Latest Notifications
            </h2>
            <Link href="/admin/notifications" className="text-[10px] font-black text-teal-600 uppercase tracking-wider hover:opacity-70 flex items-center gap-1">
              View All <ArrowRight size={12} />
            </Link>
          </div>
          {loading ? (
            ...
            <div className="text-center py-6"><Bell size={28} className="mx-auto text-gray-200 mb-2" /><p className="text-sm text-gray-400">No notifications</p></div>
          ) : (
              ...
              {recentNotifications.map(n => (
                <div key={n._id} className={`flex items-center gap-3 p-3 rounded-xl border transition-colors ${!n.isRead ? 'bg-teal-50/50 border-teal-100' : 'bg-gray-50/50 border-gray-100'}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-gray-400">{new Date(n.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}</p>
                  </div>
      ...
      {visibleQuick.length > 0 && (
        <div className="bg-white border border-gray-100 rounded-2xl p-6">
          <h2 className="font-black text-gray-900 mb-5 flex items-center gap-2">
            <div className="w-2 h-5 bg-purple-500 rounded-full" /> Quick Actions
          </h2>
